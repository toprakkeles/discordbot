...
exports.run = (client, message, args) => {
  ...
  let modlog = guild.channels.find('name', 'cezalar');
  if (!modlog) return message.reply('`cezalar adli ` kanalı oluşturman lazım.');
  if (reason.length < 1) return message.reply('Sebep ve @Kisi');
